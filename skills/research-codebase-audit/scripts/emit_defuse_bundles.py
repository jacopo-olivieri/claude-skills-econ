#!/usr/bin/env python3
"""Emit mechanical Stata definition/use candidates for b4 recheck.
...
Usage:
    emit_defuse_bundles.py PACKAGE_ROOT [--audit-dir audit] [-o OUTPUT.md]
"""
...
def _is_audit_workspace(path):
    try:
        return ((path / "audit_readme.md").is_file()
                or (path / "_run" / "manifest.json").is_file())
    except OSError:
        return False
...
def scan_package(root):
    ...
    if do_root.is_dir():
        for path in sorted(do_root.rglob("*.do")):
            ...
            ancestors = [root.joinpath(*rel_parts[:i])
                         for i in range(1, len(rel_parts) + 1)]
            if any(_is_audit_workspace(p) for p in ancestors):
                continue
            files.append(path)
...
TABLE_HEADER = (
    "| Bundle ID | Identity Tuple | Variable | Producer Shape | Definition Site | "
...
def _render_table(lines, rows, empty_line):
    ...
    for b in rows:
        ...
        cells = [f"`{b['id']}`", f"`{identity}`", b["variable"],
...
def render_artifact(results):
    ...
    lines = [
        ...
        "Generated by `scripts/emit_defuse_bundles.py` at b4. This is a mechanical",
        "candidate emitter; recheck decides semantic intent. Every standard Bundle ID",
        "must be mapped exactly once in `code_error_recheck_plan.md`. Recode rows are",
...
def main():
    ...
    try:
        ...
        return 2
    out = args.output or args.audit_dir / "_run" / "defuse_bundles.md"
